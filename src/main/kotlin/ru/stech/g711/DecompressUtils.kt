...
private val ulawtable = intArrayOf(
...
fun decompressFromG711(inpb: ByteArray, useALaw: Boolean): ByteArray {
    val table: IntArray = if (useALaw) alawtable else ulawtable
    var off = 0
    var value: Int
    val out = ByteArray(inpb.size shl 1)
    for (element in inpb) {
        value = table[element.toInt() and 0x00FF]
        out[off++] = (value shr 8 and 0x00FF).toByte()
        out[off++] = (value and 0x00FF).toByte()
    }
    return out
}